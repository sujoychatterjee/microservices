...
(function($) {
    $.flexbox = function(div, o) {
        ...
        //PATCH: (http://flexbox.codeplex.com/discussions/251643)
        if ( o.selectBehavior) { o.selectFirstMatch = false; $input.click(arrowClick); }
        ...
        var newTop = offset.top + $input.outerHeight();
            ...
            if ((/27$|38$|33$|34$/.test(e.keyCode) && $ctr.is(':visible')) ||
                ...
                switch (e.keyCode) {
                    ...
                    case 13: // enter
                        ...
                        else if(!o.skipFiltering) flexboxDelay(true);
                        break;
                    case 27: //	escape
                        hideResults();
                        break;
...
//						else callback(o.source);
		...
		function filter(data, params) {
			...
			for (var i=0; i < data[o.resultsProperty].length; i++) {
				...
				if (o.skipFiltering || (o.matchAny && indexOfMatch !== -1) || (!o.matchAny && indexOfMatch === 0)) {
        ...
        function displayItems(d, q) {
          ...
          // position the options display element
          var inputOffset = $input.offset();
          var inputWidth = $input.outerWidth() - 1;
          var ctrHeight = $ctr.height();
          var underTop = inputOffset.top + $input.outerHeight() - 1;
          var underLeft = inputOffset.left;
          var aboveTop = inputOffset.top - ctrHeight - 1;

          // can be shown below the viewport?
          var viewportHeight = $(window).height();

          if(underTop + ctrHeight > viewportHeight && aboveTop >= 0) {

            $ctr.css('width', inputWidth)
              .css('top', aboveTop)
              .css('left', underLeft);
          }
          else {
          ...
          //$ctr.parent().css('z-index', 999950);
          $ctr.css('z-index', 999950);
          $ctr.show();
    ...
    $.fn.flexbox.defaults = {
        ...
        selectBehavior: true, // set to false to disallow the select behavior //PATCH: (http://flexbox.codeplex.com/discussions/251643)
        skipFiltering: false, // set to true to show all select values regardless of the input text //PATCH: SUMO-5246
        selectElemCb: null,
        paging: {
  ...
  $.fn.destroy = function(val) {
      var id = '#' + this.attr('id');
      $(id).empty();
      // Our customization appends the ctr to the body, so remove it here.
      $("div" + id + "_ctr").remove();
  };
})(jQuery);