  ...
  DynaTreeNode.prototype = {
    ...
    _getInnerHtml:function () {
      ...
      if (level < opts.minExpandLevel) {
        ...
        res += cache.tagExpander; //TODO: Martin - I modified this because I don't need connectors
      }
      // Checkbox mode
...
//		this.tree.logDebug("dtnode.focus(): %o", this);
      this.makeVisible();
      try {
        $(this.span).find(">.dynatree-title").focus();
      } catch (e) {
      }
    },
...
//		}
    ...
    _loadKeyPath:function (keyPath, callback) {
      ...
      if (this.childList) {
        ...
        tree.logDebug("%s._loadKeyPath(%s) -> reloading %s...", this, keyPath, this);
        var self = this;
        this.reloadChildren(function (node, isOk) {
          // After loading, look for direct child with that key
          if (isOk) {
            tree.logDebug("%s._loadKeyPath(%s) -> reloaded %s.", node, keyPath, node);
            callback.call(tree, self, "loaded");
            node._loadKeyPath(keyPath, callback);
          } else {
            tree.logWarning("%s._loadKeyPath(%s) -> reloadChildren() failed.", self, keyPath);
            callback.call(tree, self, "error");
          }
        });
        // we can ignore it, since it will only be exectuted once, the the loop is ended
        // See also http://stackoverflow.com/questions/3037598/how-to-get-around-the-jslint-error-dont-make-functions-within-a-loop
...
//		if( parseFloat($.ui.version) < 1.8 ) {
    ...
    bind:function () {
      ...
      if (div.addEventListener) {
        div.addEventListener("focus", this.__focusHandler, true);
        div.addEventListener("blur", this.__focusHandler, true);
      } else {
        div.onfocusin = div.onfocusout = this.__focusHandler;
      }
      // EVENTS
...
//			this.$tabs.bind("click.tabs", function() { return false; });
    ...
    unbind:function () {
      ...
      if (div.removeEventListener) {
        div.removeEventListener("focus", this.__focusHandler, true);
        div.removeEventListener("blur", this.__focusHandler, true);
      } else {
        div.onfocusin = div.onfocusout = function () {
        };
      }
